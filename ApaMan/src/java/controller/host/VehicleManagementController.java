 ...
 * @author Laputa
 */
@WebServlet(name = "VehicleManagementController", urlPatterns = {"/vehicle"})
...
public class VehicleManagementController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (request.getParameter("submitType")) {
            case "getAllByTenant":
                int tenantId = Integer.parseInt(request.getParameter("tenantId"));
                List<Vehicle> vehicles = new VehicleService().getAll(tenantId, apartmentId);
                String vehiclesJsonString = new Gson().toJson(vehicles);

                response.setContentType("application/json");
                response.setCharacterEncoding("UTF-8");
                out.print(vehiclesJsonString);
                out.flush();
                break;
            case "delete":
